...
# ============================================================================
# Local LUKS lifecycle (data-at-rest on this host)
# ============================================================================
local_prepare_luks() {
  local crypt_file="$1"
  local mapper_name="$2"
  ...
  prepare_luks_mount \
    "ssh -q -T -o LogLevel=ERROR nas-core 'cat /volume1/NetBackup/.ash'" \
    "$crypt_file" \
    "$mapper_name" \
    "/home/tmp_crypt"
}
...
run_nas_backups_on_web_mail() {
  local log_file="/var/log/05_backupNasCore_WebMail.log"
  local start_time

  CURRENT_JOB_NAME="NasCore_on_WebMail"
  start_time="$(date +%s)"
  begin_job "$log_file"
  ...
  plain_section \
    ...
    "ssh -q -o LogLevel=ERROR -p 1622 -T -x" \
    "/home/Photos/" \
    "root@web-mail.example.org:/home/Backup_NasCore/Photos/" \
    "--delay-updates"

  dual_luks_section \
    "Backup NasCore on web-mail" \
    "$log_file" \
    "ssh -q -o LogLevel=ERROR -p 1622 -T -x" \
    "/home/tmp_crypt/" \
    "root@web-mail.example.org:/home/tmp_crypt/" \
...
# ---- mx-secondary rootfs → nas-core (LAN) ----
run_rsync_plain \
  "MX2_on_NasCore" \
  "/var/log/03_backupMX2_NasCore.log" \
  "ssh -q -o LogLevel=ERROR -p 1249 -T -x" \
  "/" \
  "root@nas-core.example.net:/volume1/MX2/" \
  "${MX2_TO_NAS_RSYNC_ARGS[@]}"

# ---- vmail spool → nas-core (decrypt-on-the-fly via local LUKS) ----
run_rsync_with_local_luks \
  "VMAIL_on_NasCore" \
  "/var/log/04_backupMail.log" \
  "ssh -q -o LogLevel=ERROR -p 1249 -T -x" \
  "/home/tmp_crypt/home/vmail/" \
  "root@nas-core.example.net:/volume1/Mail/" \
